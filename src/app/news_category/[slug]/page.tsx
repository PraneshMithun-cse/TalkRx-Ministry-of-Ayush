...
const NEWS_CATEGORIES = archives.news_category as Record<string, { title: string; items: { title: string; href: string; date: string }[] }>;

export function generateStaticParams() {
  return Object.keys(NEWS_CATEGORIES).map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const cat = NEWS_CATEGORIES[slug];
